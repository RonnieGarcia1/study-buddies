    ...
    const updateGroup = async(gid, group) => {
        try{
            const updatedGroup = await db.one(
                ...
                [group.name, group.main_focus, group.date_formed, group.contact_email, gid]
            );
            return updatedGroup;
        } catch(err){
            return err;
    ...
    const createEvent = async (event) => {
      try {
        const newEvent = await db.one(
          ...
          [event.name, event.virtual_meeting_link, event. start_time, event.end_time, event.number_of_attendees, event.study_group_id ]
        );
        return newEvent;
      } catch(error) {
        return error;
